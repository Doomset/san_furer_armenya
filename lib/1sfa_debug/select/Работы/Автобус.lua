...
return
{
    ...
    function()
        ...
        local enter_buss = function(id, hp) SendSync{pos = mimic_pos, manual = "vehicle", id = id, vehicleHealth = hp, quaternion = quat} end --отправить данные серверу о том, что чар автобусе для получения диалога
        ...
        local block_messages = function () -- 'Больше всего народу смогло укрыться в метро.'
            ...
            handler('onServerMessage', {text = 'Вы проиграли, но получаете бонус 1000', color = 267386880}, 30)
        end
        ...
        local send_pos = function (veh_id, hp, handle)
            for s = 1, #t do
                if (s == #t)  then -- , quaternion = quat -- все сног кроме первой
                    
                    wait(600)
                    SendSync{pos = {t[s][1], t[s][2], t[s][3]}, manual = "player", surf = 2034} -- забираем её с ног, чтоб не кикнуло за автобус
